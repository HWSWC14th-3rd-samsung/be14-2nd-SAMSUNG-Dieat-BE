...
-- -----------------------------------------------------
-- 2) CREATE TABLE (부모 → 자식 순, auto_increment 포함)
-- -----------------------------------------------------

-- 2-1) tbl_personal_info (부모; tbl_user_info가 참조)
...
-- 2-2) tbl_user_info (자식; FK: tbl_personal_info.user_code)
...
-- 2-4) tbl_bookmark_category (부모; tbl_bookmark가 참조)
...
-- 2-5) tbl_success_post (부모; tbl_success_meal가 참조)
...
-- 2-6) tbl_meal (부모; tbl_meal_food, tbl_success_meal가 참조)
...
-- 2-7) tbl_diet_post (부모; tbl_diet_food가 참조)
...
-- 2-9) tbl_subscribe (자식; FK: tbl_user_info)
...
-- 2-10) tbl_block (자식; FK: tbl_user_info)
...
-- 2-11) tbl_like (자식; FK: tbl_user_info)
CREATE TABLE `tbl_like` (
                            `likes_target_code` INT                NOT NULL,
                            `likes_target_type` ENUM('type1','type2') NOT NULL,
                            `user_code`         INT                NOT NULL,
...
-- 2-12) tbl_bookmark (자식; FK: tbl_bookmark_category, tbl_user_info)
CREATE TABLE `tbl_bookmark` (
                                `bm_post_type` TINYINT NOT NULL,
...
-- 2-13) tbl_user_data_food_review (자식; FK: tbl_user_data_food, tbl_user_info)
CREATE TABLE `tbl_user_data_food_review` (
                                             `rvw_user_code` INT NOT NULL,
                                             `udf_code`      INT NOT NULL,
                                             `rvw_type`      ENUM('GOOD','NEED UPDATE') NOT NULL,
                                             ...
                                             CONSTRAINT `FK_tbl_user_data_food_TO_tbl_user_data_food_review_2`
                                                 FOREIGN KEY (`rvw_user_code`)
                                                     REFERENCES `tbl_user_info` (`user_code`)
);

-- 2-14) tbl_weight (자식; FK: tbl_user_info)
...
-- 2-15) tbl_meal_food (자식; FK: tbl_meal)
...
-- 2-16) tbl_success_meal (자식; FK: tbl_meal, tbl_success_post)
...
CREATE TABLE `tbl_point` (
                             ...
                             CONSTRAINT `FK_tbl_point_TO_diet`
                                 FOREIGN KEY (`point_user_code`)
                                     REFERENCES `tbl_user_info` (`user_code`)
);

-- 2-19) tbl_free_post (독립)
...
-- 2-20) tbl_diet_food (자식; FK: tbl_diet_post)
...
CREATE TABLE `tbl_report` (
                              ...
                              `rpt_target_type`    ENUM('TYPE1','TYPE2') NOT NULL,
...
CREATE TABLE `tbl_qna_comment` (
                                   ...
                                   CONSTRAINT `FK_TBL_USER_INFO_TO_QNA_COMMENT`
                                       FOREIGN KEY (`user_code`)
                                           REFERENCES `tbl_user_info` (`user_code`)
);
